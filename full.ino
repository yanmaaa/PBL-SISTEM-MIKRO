...
void controlKurasAir(int distance) {
    if (digitalRead(PUMP1_PIN) == HIGH && distance <= JARAK_AIR_HABIS_CM) {
        // Air Penuh: Matikan P2, Akhiri Siklus
        digitalWrite(PUMP1_PIN, HIGH);
        Serial.println("Kuras Selesai.");
        lcd.clear();
        lcd.setCursor(0, 0); lcd.print("KURAS SELESAI!");
        delay(3000); 
    ...
    if (digitalRead(PUMP2_PIN) == HIGH && distance >= JARAK_AIR_PENUH_CM) {
        // Air Habis: Matikan P1, Nyalakan P2
        digitalWrite(PUMP2_PIN, HIGH); 
        Serial.println("Air Penuh");
        lcd.clear();
        lcd.setCursor(0, 0); lcd.print("AIR PENUH");
        delay(3000); 
    }
    
    // Tampilan saat Kuras Aktif
    lcd.setCursor(0, 0); 
    ...
    lcd.setCursor(0, 1); lcd.print("Jarak: "); lcd.print(distance); lcd.print(" cm");
    lcd.print("            "); 
}
